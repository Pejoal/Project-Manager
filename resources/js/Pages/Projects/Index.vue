...
<template>
  ...
  <AppLayout>
    ...
    <div class="p-2 my-1 bg-white dark:bg-gray-800 rounded-lg shadow-md">
      <CreateProjectModal :show="showModal" :statuses="statuses" :priorities="priorities" @close="closeModal" />
      <button @click="openModal" class="text-blue-500 dark:text-blue-400 hover:underline">Create Project</button>
      ...
      <ul class="my-2 space-y-4">
        ...
        >
          <div>
            <Link :href="route('projects.show', project.slug)" class="text-blue-500 dark:text-blue-400 hover:underline">
              {{ project.name }}
            </Link>
            ...
            <div class="text-gray-500 dark:text-gray-400 text-sm">Number of Tasks: {{ project.tasks_count }}</div>
            <div class="text-gray-500 dark:text-gray-400 text-sm">
              Created at: {{ new Date(project.created_at).toLocaleString() }}
            </div>
          </div>
          <div class="text-gray-500 dark:text-gray-400 text-sm">Project ID: {{ project.id }}</div>
        </li>
      </ul>
    </div>
  </AppLayout>
